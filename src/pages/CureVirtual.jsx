    ...
    <section className="relative min-h-[90vh] flex items-center overflow-hidden pt-32 pb-20 bg-bg">
      ...
      <div className="container-section relative z-10 text-center flex flex-col items-center">
        <motion.div variants={fadeUp} initial="hidden" animate="show" className="max-w-4xl w-full">
          ...
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-cure-green/10 border border-cure-green/20 text-xs font-mono uppercase tracking-wider text-cure-green mb-8 font-semibold shadow-sm ml-3">
          ...
          <h1 className="text-5xl sm:text-6xl lg:text-7xl font-display font-bold tracking-tight leading-[1.05] text-cure-navy">
            ...
            <span className="text-cure-orange">how far</span> the nearest
            clinic is.
          </h1>
          <p className="mt-8 text-lg sm:text-xl text-text-secondary max-w-2xl mx-auto leading-relaxed font-medium">
            CureVirtual connects patients, doctors, physician assistants,
            laboratories and pharmacies into one coordinated telehealth
...
function ProblemStatement() {
  const stats = [
    { value: "1:5,000+", label: "patients per doctor in underserved regions" },
    { value: "40km+", label: "average distance to the nearest specialist" },
    { value: "60%", label: "of referrals never reach the specialist" },
  ];
  return (
    <section className="section-padding bg-surface border-t border-card-border">
      <div className="container-section text-center">
        ...
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-display font-bold text-cure-navy tracking-tight max-w-2xl mx-auto">
          Specialist care is concentrated where population isn't.
        </h2>
        ...
        <div className="mt-16 grid sm:grid-cols-3 gap-6">
          {stats.map((s, i) => (
            <motion.div
              ...
              className="text-center rounded-3xl glass border-card-border py-12 px-8 group hover:border-cure-navy/20 transition-all duration-300 bg-white"
            >
              <span className="text-4xl sm:text-5xl font-display font-bold text-cure-navy group-hover:scale-110 transition-transform duration-500 inline-block">
                {s.value}
              </span>
              <p className="mt-4 text-[15px] font-medium text-text-muted">{s.label}</p>
            </motion.div>
          ))}
        </div>
      </div>
          ...
          {CURE_MODULES.map((m, i) => {
            ...
            return (
              <motion.div
                ...
                viewport={viewportOnce}
              >
                <GlassCard className="p-8 h-full group bg-white border border-card-border hover:border-cure-navy/30">
                  <div className="w-14 h-14 rounded-2xl bg-surface border border-card-border flex items-center justify-center mb-6 group-hover:scale-110 transition-all duration-300">
                    <Icon size={26} className="text-cure-navy" />
                  </div>
                  <h3 className="text-xl font-bold text-cure-navy tracking-tight">
                    {m.title}
                  </h3>
    ...
    <section className="section-padding bg-surface border-y border-card-border relative overflow-hidden">
      ...
      <div className="container-section relative grid lg:grid-cols-2 gap-16 items-center z-10">
        <motion.div variants={slideRight} initial="hidden" whileInView="show" viewport={viewportOnce}>
          <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-cure-green/10 border border-cure-green/20 text-xs font-mono uppercase tracking-wider text-cure-green mb-6 font-semibold">
          ...
          <h2 className="text-4xl sm:text-5xl font-display font-bold text-cure-navy leading-[1.1] tracking-tight">
            Launching first in Ghana, in partnership with regional health
            networks.
          </h2>
        ...
        <motion.div variants={fadeIn} initial="hidden" whileInView="show" viewport={viewportOnce}>
          <div className="rounded-3xl bg-white border border-card-border shadow-sm p-10">
            <div className="grid grid-cols-2 gap-8">
              {[
                { label: "Launch region", value: "Greater Accra" },
                { label: "Partner clinics", value: "18 at launch" },
                { label: "Target reach", value: "250k+ patients" },
                { label: "Modules live", value: "All 5" },
              ].map((item) => (
                <div key={item.label}>
                  <p className="text-xs font-mono uppercase tracking-[0.1em] text-text-muted font-semibold">
                    {item.label}
                  </p>
                  <p className="mt-2 text-3xl font-display font-bold text-cure-navy">
                    {item.value}
                  </p>
                </div>
              ))}
      ...
      <div className="container-section">
        ...
        <div className="mt-16 grid md:grid-cols-3 gap-8 items-stretch">
          {CURE_PRICING.map((tier, i) => (
            ...
            >
              <div
                className={`h-full rounded-3xl p-10 flex flex-col transition-all duration-300 bg-white ${
                  tier.highlighted
                    ? "border-2 border-cure-navy shadow-[0_10px_30px_rgba(18,63,99,0.1)] scale-[1.03]"
                    : "border border-card-border shadow-sm"
                }`}
              >
                {tier.highlighted && (
                  <span className="self-start mb-6 text-xs font-mono uppercase tracking-wider px-3 py-1.5 rounded-full bg-cure-navy/10 text-cure-navy font-bold">
                    Most popular
                  </span>
                )}
                ...
                <div className="mt-5 flex items-baseline gap-2">
                  <span className="text-4xl font-display font-bold text-cure-navy">
                    {tier.price}
                  </span>
                ...
                <ul className="mt-8 space-y-4 flex-1">
                  {tier.features.map((f) => (
                    <li key={f} className="flex items-center gap-3 text-[15px] font-medium">
                      <Check
                        size={18}
                        className={tier.highlighted ? "text-cure-navy" : "text-text-muted"}
                      />
...
function BenefitsSection() {
  const benefits = [
    { title: "Fewer missed referrals", description: "Structured hand-offs between roles mean referrals don't disappear in translation." },
    { title: "Faster time to diagnosis", description: "Lab results route directly into the doctor's view — no faxes, no phone calls." },
    { title: "Lower cost per consultation", description: "Video-first triage reduces unnecessary in-person visits for both patients and clinics." },
  ];
  return (
    <section className="section-padding bg-surface border-y border-card-border">
      <div className="container-section grid md:grid-cols-3 gap-8">
        {benefits.map((b, i) => (
          <motion.div
            ...
            viewport={viewportOnce}
          >
            <div className="p-8 bg-white shadow-sm rounded-2xl border border-card-border h-full hover:border-cure-navy/30 transition-all duration-300">
              <ShieldCheck size={28} className="text-cure-green mb-5" />
              <h3 className="text-xl font-bold text-cure-navy tracking-tight">{b.title}</h3>
              <p className="mt-3 text-[15px] text-text-secondary font-medium leading-relaxed">
                {b.description}
              </p>
            </div>
          ...
          >
            {cureTestimonials.map((t) => (
              <SwiperSlide key={t.id}>
                <GlassCard className="p-8 h-full flex flex-col group bg-white border border-card-border" tilt={false}>
                  ...
                  <div className="mt-8 pt-6 border-t border-card-border flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-cure-navy text-white flex items-center justify-center font-display font-bold">
                      {t.name[0]}
                    </div>
                    <div>
                      <p className="font-bold text-cure-navy text-sm">{t.name}</p>
                      <p className="text-xs font-mono uppercase tracking-wider text-text-muted mt-1">{t.role}</p>
                    </div>
                  </div>
                </GlassCard>
              </SwiperSlide>
  ...
  return (
    <section id="faq" className="section-padding bg-surface border-t border-card-border">
      <div className="container-section max-w-3xl">
        <h2 className="text-3xl font-display font-bold text-cure-navy mb-12">
          Questions about CureVirtual
        </h2>
        ...
        >
          ...
          <div className="relative z-10">
            <h2 className="text-4xl sm:text-5xl lg:text-6xl font-display font-bold text-white leading-[1.1] tracking-tight max-w-3xl mx-auto">
              Be first in line when CureVirtual launches in your region.
            </h2>
