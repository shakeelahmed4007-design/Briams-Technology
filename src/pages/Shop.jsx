        ...
        <div className="container-section relative z-10 text-center flex flex-col items-center">
          <motion.div variants={fadeUp} initial="hidden" animate="show" className="max-w-3xl">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-display font-bold text-text-primary tracking-tight">
              Pre-built enterprise software & blueprints.
            </h1>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {SHOP_ITEMS.map((item, index) => (
              ...
              >
                <div>
                  {item.popular && (
                    <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-briams-orange text-white text-[11px] font-mono uppercase tracking-wider font-semibold px-3 py-1 rounded-full shadow-md">
                      Most Popular
                    </span>
                  )}
                  <span className="text-xs font-mono font-semibold uppercase tracking-wider text-briams-blue">
                    {item.category}
                  </span>
                    ...
                    <ul className="mt-6 space-y-3">
                      {item.features.map((feat, i) => (
                        <li key={i} className="flex items-center gap-2.5 text-sm text-text-secondary font-medium">
                          <Check size={16} className="text-briams-orange shrink-0" />
                          <span>{feat}</span>
                        </li>
                      ))}
                    </ul>
